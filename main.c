...
float k;
int   structSize;                                 // for allocating memory                           	                   // order fixed pric
...
aProblem *read_csv(char *filename, aProblem *p);
void callWW(aProblem *pb,Chromosom *solution,float Qt[]);
int callUser();
/*===========================================================================
 |                     Global Vars: change program parameters from here                                     |
=============================================================================*/
int    numOfGenerations =100;                       // change here the number of generations to process
...
float Qt[t];                                         // an array for solution
for (i=0;i<t;i++) Qt[i]=0;                           // init array
// display Default data:------------------------------------------------------------
   printf("\nGeneticAlgoritem implemention:\nlooking a sulution for problem:\n-------------------------------------------------------------------------------------------\n");
   printf("Number of periods=%d\t holdCost =%4.1f\t\tinvititionCost=%4.1f ",t,h,k);
   printf("\n\nDemands:\n");
   for (i=0;i<t;i++) printf("period %d: [%d] ",i+1,Demands[i]);
   //("\n\n\n");
//----------------------------------------------------------------------------------------------------------------------
//Ask user for preferancess:
aProblem *pb;
int option=callUser();
if ( option == 2) pb=readCsv("input.csv",pb);        // creatig a problem data record (read from file)
else if (option ==1 )
  pb = createProblem(pb,t,h,k,Demands);              // creatig a problem data record (init with defaults)
else return 0;                                       // user program Termination
if (pb==NULL) {
  printf("Error opening problem Record: struct aProblem");
  return 0;                                          // error creating pointer to the data
}
...
populationSize=pb->t;                                                   // population: number of online active soulutions. by default = t
...
//=============================================================================================================================================================================//
void callWW(aProblem *pb1,Chromosom *solution1,float Qt1[]){
int i=0;
printf("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
printf("\nCalculate optimal solution according Wagner within algoritem:\n");
printf("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
float wwaTC= WWA(pb1->D,pb1->t,pb1->h,pb1->k,Qt1);                   // Call WW algoritem - calc the optimal soulution


for (i=0;i<pb1->t;i++)printf("\nperiod %d buy:%4.0f",i+1,Qt1[i]);
printf("\n");
printf ("WWA optimal Total Cost TC=%4.0f$",wwaTC);   
  ...
  // printing solution in to file: \\solution.csv
  // this file will store in the current active directory
save2File("solution.csv",solution1,Qt1,wwaTC,pb1 );
printf("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
